...
void uart_init(void)
{
	...
	/*Initialization uart	*/
    uart_disable(UART1);     /*close uart	*/
	uart_softreset(UART1);	/*reset UART1*/
	...
	UART1->UFCR = 5 << 7;				/*1分频，uart1_clk=80Mhz*/
	UART1->UBIR = 71;
	UART1->UBMR = 3124;
#endif
...
{
	while(((UART1->USR2) & 0x01) == 0);			/*等待有数据后再读取*/
	return UART1->URXD;
}
